...
function createObj() {
    ...
    orderDetails.forEach(item => {
        ...
        // FIX số
        obj.price = parseInt(item.price) || 0;
        obj.quantity = parseInt(item.soluong) || 0;
...
// =================================================================
// 1. CHỨC NĂNG QUẢN LÝ MÃ GIẢM GIÁ (Thêm, Xóa, Hiển thị)
// =================================================================

// Khởi tạo danh sách mã giảm giá (Nếu chưa có trong localStorage)
function createDiscountCodes() {
    if (localStorage.getItem('discountCodes') == null) {
        const codes = [
            // Mã ví dụ 1: Giảm 15% (percent), hết hạn cuối năm 2025, tối thiểu 2 món
            { name: 'NEWUSER', value: 15, type: 'percent', expiry: '2025-12-31', min_items: 2 }, 
            // Mã ví dụ 2: Giảm 30,000 VND (fixed), hết hạn cuối tháng 10/2025, tối thiểu 1 món
            { name: 'MINFOOD30K', value: 30000, type: 'fixed', expiry: '2025-10-30', min_items: 1 } 
        ];
        localStorage.setItem('discountCodes', JSON.stringify(codes));
    }
}

// Hàm hiển thị/ẩn form tạo mã giảm giá
...
// Hàm hiển thị danh sách mã giảm giá ra bảng
function showDiscountCodes() {
    createDiscountCodes(); // Đảm bảo danh sách được khởi tạo lần đầu
    const codes = JSON.parse(localStorage.getItem('discountCodes') || '[]');
    const tableBody = document.getElementById('discountCodeTableBody');
    let html = '';
    
    codes.forEach((code) => {
        // Định dạng giá trị
        const valueDisplay = code.type === 'percent' ? `${code.value}%` : vnd(code.value);
        
        // Kiểm tra mã hết hạn
        const today = new Date().toISOString().split('T')[0];
        const expired = code.expiry < today;
        const rowClass = expired ? 'style="background-color: #ffe0e0; opacity: 0.6;"' : '';
        
        html += `
            <tr ${rowClass}>
                <td>${code.name}</td>
                <td>${valueDisplay}</td>
                <td>${code.type === 'percent' ? 'Phần trăm' : 'Cố định'}</td>
                <td>${code.expiry} ${expired ? ' <span style="color: red;">(Hết hạn)</span>' : ''}</td>
                <td>${code.min_items}</td>
                <td>
                    <button class="btn-delete" onclick="deleteDiscountCode('${code.name}')">
...
// Hàm xóa mã giảm giá
function deleteDiscountCode(codeName) {
    let codes = JSON.parse(localStorage.getItem('discountCodes') || '[]');
    const index = codes.findIndex(c => c.name === codeName);
    
    if (index !== -1 && confirm(`Bạn có chắc muốn xóa mã giảm giá "${codeName}"?`)) {
        codes.splice(index, 1);
        localStorage.setItem('discountCodes', JSON.stringify(codes));
        toast({ title: 'Thành công', message: `Đã xóa mã ${codeName} thành công!`, type: 'success', duration: 3000 });
        showDiscountCodes(); // Cập nhật lại bảng
    }
}

// Lắng nghe sự kiện submit form tạo mã giảm giá mới
document.getElementById('discountCodeForm').addEventListener('submit', function(e) {
    e.preventDefault();
    
    // Lấy giá trị từ input
    const codeName = document.getElementById('codeName').value.trim().toUpperCase();
    const codeValue = parseInt(document.getElementById('codeValue').value);
    const codeType = document.getElementById('codeType').value;
    const codeExpiry = document.getElementById('codeExpiry').value;
    const minItems = parseInt(document.getElementById('minItems').value);
    
    // Kiểm tra tính hợp lệ cơ bản
    if (!codeName || isNaN(codeValue) || !codeExpiry || isNaN(minItems) || codeValue <= 0 || minItems <= 0) {
        toast({ title: 'Lỗi', message: 'Vui lòng điền đầy đủ và chính xác thông tin!', type: 'error', duration: 3000 });
        return;
    }
    ...
    if (codes.some(c => c.name === codeName)) {
        toast({ title: 'Lỗi', message: `Mã giảm giá "${codeName}" đã tồn tại!`, type: 'error', duration: 3000 });
        return;
    }
    
    // Tạo đối tượng mã mới
    const newCode = {
        name: codeName,
        value: codeValue,
        type: codeType,
    ...
    toast({ title: 'Thành công', message: `Tạo mã ${codeName} thành công!`, type: 'success', duration: 3000 });
    
    // Reset form, ẩn form và cập nhật bảng
    this.reset();
    toggleCreateForm();
    showDiscountCodes();
});
